...
| <<plugins-{type}s-{plugin}-cluster>> | <<array,array>>|No
| <<plugins-{type}s-{plugin}-replica>> | <<boolean,boolean|>>No
...
[id="plugin-{type}s-{plugin}-cluster"]
===== `cluster`

  * Value type is <<array,array>>
  * There is no default value for this setting.
  * Cluster will override Host and Path configuration if both specified

The array of nodes of redis cluster. Details can be found in https://github.com/redis/redis-rb#cluster-support[redis-rb]. 

[id="plugin-{type}s-{plugin}-replica"]
===== `replica`

  * Value type is <<boolean,boolean>>
  * Default value is `false`.
  * Replica will be used while Cluster has value.

Enable read only from any replica. Details can be found in https://github.com/redis/redis-rb#cluster-support[redis-rb]. 
